...
const GISTS = "/factory-arrow-functions-in-js/"

export default () => (
	<BlogPostLayout post={meta}>
		<p>
			This article describes an alternative approach to instantiating objects from a template in
			JavaScript. For better comprehension, it is sometimes compared with commonly used ES6 classes.
		</p>
		...
		<ol class="list-inside list-decimal">
			...
			<li>
				Subclassing is possible using the <code>extends</code> keyword, referencing parent class is
				possible using the <code>super</code> keyword
			</li>
			<li>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-1.js")}
		/>
		...
		<p>
			...
			like to share here. In JavaScript, we can create a function that accepts arguments and returns
			an object that has exclusive access to those arguments via closure.
		</p>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-2.js")}
		/>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-3.js")}
		/>
		...
		<p>
			...
			<code>this</code>, we can benefit from using the closure over the arguments. As the object has
			access to the arguments, it means that they are available in its methods.
		</p>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-4.js")}
		/>
		...
		<p>
			It is impossible to directly change the arguments passed to a function from the outside. They
			cannot be accessed, and they cannot be changed. You can explicitly allow access by binding the
			arguments to object properties. In the example below, <code>length</code> is available on the
			object externally, but <code>width</code> only exists inside and there is no way to access it
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-5.js")}
		/>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-6.js")}
		/>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-7.js")}
		/>

		<Heading level={3}>No Direct Inheritance And Internal Method Calls</Heading>

		<p>
			If you looked at the previous example, you probably noticed that length is multiplied by width
			in two places: in <code>getArea</code> and in <code>getTotalAreaWith</code>. This is because
			we cannot use <code>this</code> and access <code>getArea</code> from inside{" "}
		...
		<p>
			But, due to the anonymous nature of our methods, we can write those separately and build up a
			horizontal extension of our objects and share methods between or even outside the objects.
		</p>
		...
		<p>
			In the example below, I create a <code>multiplyThunk</code> that is partially applied with two
			values. I then assign it as a <code>getArea</code> method on multiple different factory arrow
			function return objects and make it work for multiple shapes with a single function:
		</p>

		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-8.js")}
		/>

		<p>
			<strong>NOTE</strong>: Using the partial application is possible in ES6 classes, but there is
			a small chance you would need to do it as you would generally prefer to use <code>this</code>{" "}
		...
		<p>
			...
			This way, we can create lightweight objects with the methods and properties we really need in
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-9.js")}
		/>
		...
		<CodeBlockFromGist
			language="javascript"
			path="rectangle.js"
			file={GISTS.concat("example-10.js")}
		/>
		...
		<p>
			...
			<a href="/blog/factory-arrow-functions-in-ts.html">In the next one</a>, I extend the topic by
			covering factory arrow function usage with TypeScript.
		</p>
	</BlogPostLayout>
)
